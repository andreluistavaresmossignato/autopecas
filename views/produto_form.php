...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header h1 { color: #1e3c72; font-size: 2.5em; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 2px; display: flex; align-items: center; justify-content: center; gap: 12px; }
        .header .subtitle { color: #666; font-size: 1.1em; }
        .form-container { max-width: 800px; margin: 0 auto; }
        .form-group { margin-bottom: 20px; }
        .form-group label { display: flex; align-items: center; gap: 8px; margin-bottom: 8px; color: #333; font-weight: 600; }
        ...
        .btn { display: inline-flex; align-items: center; gap: 8px; padding: 12px 30px; background: #ff6b35; color: white; border: none; border-radius: 5px; font-size: 1em; font-weight: 600; cursor: pointer; text-decoration: none; text-align: center; transition: all 0.3s; text-transform: uppercase; letter-spacing: 1px; }
...
<body>
    <div class="container">
        <div class="header">
            <h1><i class="bi bi-gear-wide-connected"></i> <?= isset($produto) ? 'Editar Peça' : 'Nova Peça' ?></h1>
            ...
            <form action="../controllers/ProdutoController.php?acao=salvar" method="POST">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="codigo_peca"><i class="bi bi-upc-scan"></i> Código da Peça *</label>
                    ...
                    <div class="form-group">
                        <label for="nome"><i class="bi bi-tag"></i> Nome da Peça *</label>
                        <input type="text" id="nome" name="nome" value="<?= htmlspecialchars($produto['nome'] ?? '') ?>" required placeholder="Ex: Filtro de Óleo">
                    </div>
                </div>
                
                <div class="form-group">
                    <label for="descricao"><i class="bi bi-text-paragraph"></i> Descrição</label>
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="preco"><i class="bi bi-currency-dollar"></i> Preço (R$) *</label>
                        <input type="number" id="preco" name="preco" step="0.01" min="0" value="<?= $produto['preco'] ?? '' ?>" required placeholder="0,00">
                    </div>
                    <div class="form-group">
                        <label for="quantidade_estoque"><i class="bi bi-box-seam"></i> Quantidade em Estoque *</label>
                ...
                <div class="form-actions">
                    <button type="submit" class="btn btn-success"><i class="bi bi-save"></i> Salvar Dados</button>
                    <a href="../controllers/ProdutoController.php?acao=listar" class="btn btn-secondary"><i class="bi bi-x-circle"></i> Cancelar</a>
                </div>
            </form>
        </div>
        
        <div class="footer">
            <p><i class="bi bi-info-circle"></i> Campos marcados com * são obrigatórios</p>
        </div>
    </div>
</body>
</html>